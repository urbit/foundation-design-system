...
(function (module, exports) {
	...
	var _getAssetPathFromRoute = _interopRequireDefault(getAssetPathFromRoute$1);

	var _requestIdleCallback = requestIdleCallback.exports;
	...
	function getFilesForRoute(assetPrefix, route) {
	  if (process.env.NODE_ENV === 'development') {
	    return Promise.resolve({
	      scripts: [assetPrefix + '/_next/static/chunks/pages' + encodeURI((_getAssetPathFromRoute).default(route, '.js'))],
...
(function (module, exports) {
	...
	var _headManager = headManager.exports;
...
escapeRegexp.escapeStringRegexp = escapeStringRegexp; // regexp is based on https://github.com/sindresorhus/escape-string-regexp
...
function getParametrizedRoute(route) {
  ...
  const parameterizedRoute = segments.map(segment => {
    if (segment.startsWith('[') && segment.endsWith(']')) {
      ...
      return `/${(_escapeRegexp$1).escapeStringRegexp(segment)}`;
    }
...
function getMiddlewareRegex(normalizedRoute, catchAll = true) {
  const result = (_routeRegex).getParametrizedRoute(normalizedRoute);
...
utils.ST = utils.SP = utils.warnOnce = void 0;
...
isDynamic.isDynamicRoute = isDynamicRoute; // Identify /[param]/ in route string
...
function denormalizePagePath(page) {
  ...
  return _page.startsWith('/index/') && !(_utils$1).isDynamicRoute(_page) ? _page.slice(6) : _page !== '/index' ? _page : '/';
}
...
function parseRelativeUrl(url, base) {
  ...
  return {
    pathname,
    query: (_querystring$1).searchParamsToUrlQuery(searchParams),
    search,
    hash,
...
function getPathMatch(path, options) {
  ...
  const matcher = (_pathToRegexp$1).regexpToFunction((options === null || options === void 0 ? void 0 : options.regexModifier) ? new RegExp(options.regexModifier(regexp.source), regexp.flags) : regexp, keys);
  /**
...
function requireResolveRewrites () {
	...
	var _normalizeTrailingSlash = normalizeTrailingSlash.exports;
	...
	function resolveRewrites$1(asPath, pages, rewrites, query, resolveHref, locales) {
	  ...
	  let fsPathname = (_normalizeTrailingSlash).removePathTrailingSlash((_normalizeLocalePath).normalizeLocalePath((_router).delBasePath(parsedAs.pathname), locales).pathname);
	  let resolvedHref;

	  const handleRewrite = rewrite => {
	    const matcher = (_pathMatch).getPathMatch(rewrite.source, {
	    ...
	    if (params) {
	      ...
	      fsPathname = (_normalizeTrailingSlash).removePathTrailingSlash((_normalizeLocalePath).normalizeLocalePath((_router).delBasePath(asPath), locales).pathname);
...
var querystring = _interopRequireWildcard(querystring$1);
...
function requireRouter$1 () {
	...
	var _routeLoader = routeLoader.exports;

	var _script = script.exports;
	...
	var _mitt = _interopRequireDefault(mitt$1);
	...
	function addPathPrefix(path, prefix) {
	  ...
	  return (_normalizeTrailingSlash).normalizePathTrailingSlash(`${prefix}${pathname}`) + path.slice(pathname.length);
	}
	...
	function resolveHref(router, href, resolveAs) {
	  ...
	  let base;
	  let urlAsString = typeof href === 'string' ? href : (_formatUrl).formatWithValidation(href); // repeated slashes and backslashes in the URL are considered
	...
	class Router {
	  constructor(pathname1, query1, as1, {
	    ...
	    const route = (_normalizeTrailingSlash).removePathTrailingSlash(pathname1); // set up the component cache (by route keys)
	  ...
	  async change(method, url, as, options, forcedScroll) {
	    ...
	    if (process.env.__NEXT_I18N_SUPPORT) {
	      ...
	      if (process.env.__NEXT_I18N_SUPPORT) {
	        ...
	        if (!((ref = this.locales) === null || ref === void 0 ? void 0 : ref.includes(nextState.locale))) {
	          ...
	          window.location.href = (_formatUrl).formatWithValidation(parsedAs); // this was previously a return but was removed in favor
	    ...
	    if (shouldResolveHref && pathname !== '/_error') {
	      ...
	      if (process.env.__NEXT_HAS_REWRITES && as.startsWith('/')) {
	        const rewritesResult = (_resolveRewrites).default(addBasePath(addLocale(cleanedAs, nextState.locale)), pages, rewrites, query, p => resolveDynamicRoute(p, pages), this.locales);
	    ...
	    */
	    ...
	    if ((!options.shallow || options._h === 1) && (options._h !== 1 || (_isDynamic).isDynamicRoute((_normalizeTrailingSlash).removePathTrailingSlash(pathname)))) {
	    ...
	    if ((_isDynamic).isDynamicRoute(route)) {
	      ...
	      const routeMatch = (_routeMatcher).getRouteMatcher(routeRegex)(asPathname);
	  ...
	  */
	  ...
	  async prefetch(url, asPath = url, options = {}) {
	    ...
	    if (process.env.__NEXT_HAS_REWRITES && asPath.startsWith('/')) {
	      ...
	      ({
	        __rewrites: rewrites
	      } = await (_routeLoader).getClientBuildManifest());
	      const rewritesResult = (_resolveRewrites).default(addBasePath(addLocale(asPath, this.locale)), pages, rewrites, parsed.query, p => resolveDynamicRoute(p, pages), this.locales);
...
function requireRouter () {
	...
	(function (module, exports) {
		...
		var _isError = _interopRequireDefault(isError$1);
		    ...
		    _router.default.events.on(event, (...args) => {
		      ...
		      if (_singletonRouter[eventField]) {
		        try {
		          ...
		          console.error((_isError).default(err) ? `${err.message}\n${err.stack}` : err + '');
		        }
		      }
		    });
		  });
...
(function (module, exports) {
	...
	function useIntersection({
	  ...
	  const [root, setRoot] = (_react).useState(rootRef ? rootRef.current : null);
...
(function (module, exports) {
	...
	var _useIntersection = useIntersection.exports;
	...
	function prefetch(router, href, as, options) {
	  ...
	  if (!(_router).isLocalURL(href)) return; // Prefetch the JSON page if asked (only in the client)
	...
	const Link = /*#__PURE__*/_react.default.forwardRef((props, forwardedRef) => {
	  ...
	  const router = (_router1).useRouter();
	  ...
	  if (!legacyBehavior || passHref || child.type === 'a' && !('href' in child.props)) {
	    ...
	    const localeDomain = router && router.isLocaleDomain && (_router).getDomainLocale(as, curLocale, router && router.locales, router && router.domainLocales);
...
(function (module) {
	module.exports = link$2.exports;
} (link$3));

var Link = /*@__PURE__*/getDefaultExportFromCjs(link$3.exports);
...
function Fence({
  ...
  return /*#__PURE__*/React__default["default"].createElement(Highlight, _extends$1({}, defaultProps, {
    key: language,
    language: language,
    code: children,
    theme: undefined
...
function MarkdownRender({
  ...
  return markdoc_default.renderers.react(content, React__default["default"], {
    components: {
      Fence,
      Tabs,
      Tab,
